...
require $theme_dir . '/custom/functions.php';
...
add_action('wp_enqueue_scripts', function () {
	$css = [
		[
			'path' => get_stylesheet_directory() . '/assets/custom/css/global.css',
			'url' => get_stylesheet_directory_uri() . '/assets/custom/css/global.css'
		]
	];

	foreach ($css as $style) {
		if (file_exists($style['path'])) {
			$css_version = filemtime($style['path']);
			$css_basename = basename($style['path'], '.css');
			wp_enqueue_style( "custom-$css_basename", $style['url'], [], $css_version);
		}
	}
});
