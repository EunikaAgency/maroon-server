...
class DSM_Advanced_Tabs_Child extends ET_Builder_Module {
		...
		$this->advanced_fields = array(
			...
			'fonts'          => array(
				'tab_title'        => array(
					...
					'css'            => array(
						'main' => '.dsm-advanced-tabs-wrapper %%order_class%%.dsm-tab .dsm-title, .dsm-advanced-tabs-wrapper %%order_class%%.dsm-tab .dsm-tab-subtitle',
					),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		',
			...
			'' !== $this->props['dsm_subtitle'] ? sprintf( '<div class="dsm-tab-subtitle">%1$s</div>', $this->props['dsm_subtitle'] ) : ''
		);
		$content_markup = '';
		...
		if ( isset( get_option( 'dsm_settings_misc' )['dsm_dynamic_assets'] ) && ! empty( get_option( 'dsm_settings_misc' )['dsm_dynamic_assets'] ) && 'on' === get_option( 'dsm_settings_misc' )['dsm_dynamic_assets'] ) {
			if ( isset( get_option( 'dsm_settings_misc' )['dsm_dynamic_assets_compatibility'] ) && ! empty( get_option( 'dsm_settings_misc' )['dsm_dynamic_assets'] ) && 'on' === get_option( 'dsm_settings_misc' )['dsm_dynamic_assets_compatibility'] ) {
				wp_enqueue_style( 'dsm-dsm-advanced-tabs-child', plugin_dir_url( __DIR__ ) . 'AdvancedTabsChild/style.css', array(), DSM_PRO_VERSION, 'all' );
			} else {
	...
	public function dsm_load_required_divi_assets( $assets_list, $assets_args, $instance ) {
		...
		$assets_list['dsm_advanced_tabs_child'] = array(
			'css' => plugin_dir_url( __DIR__ ) . 'AdvancedTabsChild/style.css',
		);
