jQuery(document).ready(function($) {
    function initCarousel($widget) {
        const carousel = $widget.find('.nlp-feature-list');
        const container = $widget.find('.nlp-carousel-container');
        const dots = $widget.find('.nlp-pagination-dot');
        ...
        $(window).on('resize', function() {
            if ($(window).width() <= 768) {
                const activeDot = $widget.find('.nlp-pagination-dot.active');
                if (activeDot.length) {
    ...
    $('.elementor-element .nlp-feature-list').each(function() {
        initCarousel($(this).closest('.elementor-element'));
    });

    // For Elementor editor preview
    if (window.elementorFrontend) {
        elementorFrontend.hooks.addAction('frontend/element_ready/feature_carousel.default', function($scope) {
            initCarousel($scope);
        });
    }
});