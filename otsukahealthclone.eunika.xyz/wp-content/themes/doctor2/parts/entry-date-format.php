{if $dateIcon}
	...
	<span class="entry-date {if $dateShort == 'yes'}short-date{/if}">
		...
		{if $dateLinks == 'yes'}
			...
			<time class="date" datetime="{$dateIcon|date: 'c'}">
				<a class="link-day" href="{$post->dayArchiveUrl}" title="{__ 'Link to daily archives: %s'|printf:''}{$dateIcon|dateI18n}">
					{$dateIcon|dateI18n: $dayFormat}{if !empty($dayFormatSuffix)}<small>{$dateIcon|date: $dayFormatSuffix}</small>{/if}
				</a>
