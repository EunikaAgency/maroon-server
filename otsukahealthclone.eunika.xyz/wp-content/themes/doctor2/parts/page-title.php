{var $customBg = $el->option('customBg')}
...
<div class="page-title"{if $titleImage} style="background-image: url('{imageUrl $titleImage, width => 1000, height => 500, crop => 1}')"{/if}>
	<div class="grid-main">

		<header class="entry-header{if $titleImage or $customBg['image'] != ""} has-image{/if}">
			...
			<div class="entry-title {$titleClass}">
				...
				<div class="entry-title-wrap">
					...
					{if $dateInterval == 'yes' or $titleCategory == 'yes' or $titleComments == 'yes' or $titleSubDesc}
			...
			{if $wp->isSingular(post) and $post->excerpt}
				<div class="page-description">{!$post->excerpt}</div>
			{/if}
		...
		{if $titleImage}
			<a href="{$titleImage}" class="thumb-link"><i class="fa fa-search"></i></a>
		{/if}
