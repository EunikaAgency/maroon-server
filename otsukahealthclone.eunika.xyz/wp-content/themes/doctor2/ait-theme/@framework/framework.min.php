...
function
onInit(){add_action('wp_enqueue_scripts',array(__CLASS__,'onEnqueueScriptsAndStyles'));add_action('admin_enqueue_scripts',array(__CLASS__,'onEnqueueAdminScriptsAndStyles'));add_action('wp_head',array(__CLASS__,'wp_head'));$isAjax=(defined('DOING_AJAX')and
DOING_AJAX===true);if(!$isAjax){AitLicensing::handleConfig();if(!AitLicensing::isOk()){if(is_admin()){AitLicensing::handleAdmin();}else{AitLicensing::handleFrontend();}}}add_action('wp',array(__CLASS__,'initWpLatte'),1999);}static
function
wp_head(){self::getManager('assets')->initGlobalFrontendJsVariables();}static
function
...
function
cleanLessCache(){AitUtils::delete(aitPaths()->dir->cache,'.ht-*.less-cache');AitUtils::delete(aitPaths()->dir->cache,'*.css');}static
function
...
AitLicensing{static
function
renderDomainControl(){if(self::isTF())return;?>
		<div class="ait-opt-label">
			<div class="ait-label-wrapper">
		...
		<div class="ait-opt ait-opt-text">
			<div class="ait-opt-wrapper">
				<input type="text" id="ait-licensing-domain" value="<?php echo
esc_attr(self::domain())?>" onclick="this.focus();this.select()" readonly style="cursor: copy;">
			</div>
			<div class="ait-help"><?php echo
wp_kses_post(__('You will need this when you will be generating API Key','ait'))?></div>
		</div>
		<?php
}static
function
renderKeyControl(){if(self::isTF()){$label=__('Purchase Code','ait');$option='tf_purchase_code';$value=self::purchaseCode();}else{$label=__('API Key','ait');$option='api_key';$value=self::apiKey();}?>
		<div class="ait-opt-label">
			<div class="ait-label-wrapper">
				<label class="ait-label" for="ait-licensing-key"><?php echo
esc_html($label)?></label>
			</div>
		</div>
		<div class="ait-opt ait-opt-text">
			<div class="ait-opt-wrapper">
				<input type="text" id="ait-licensing-key" name="_ait_updater_options[<?php echo
esc_attr($option)?>]" value="<?php echo
esc_attr($value)?>">
			</div>
			<div class="ait-help">
			<?php if(!self::isTF()):?>
				<?php echo
wp_kses_post(sprintf(__('You can generate API Key for the domain in your %sAitThemes account%s.','ait'),'<a href="https://system.ait-themes.club/account/api" target="_blank">','</a>'))?>
			<?php else:?>
				<?php echo
wp_kses_post(sprintf(__('You can find Purchase Code for this theme in your %sThemeForest account &rarr; Downloads%s under the Download button.','ait'),'<a href="https://themeforest.net/downloads" target="_blank">','</a>'))?>
			<?php endif?>
			</div>
		</div>
		<?php
...
static
function
domainUsed(){$o=get_transient('check_ait_subscription');if(!empty($o->body['domain'])){return$o->body['domain'];}}protected
static
function
purchaseCode(){$o=get_option('_ait_updater_options');return!empty($o['tf_purchase_code'])?$o['tf_purchase_code']:'';}protected
static
function
apiKey(){$o=get_option('_ait_updater_options');return!empty($o['api_key'])?$o['api_key']:'';}static
function
handleConfig($value=''){add_filter('ait-get-full-config',function($config,$type){if($type!=='theme')return$config;if(isset($config['licensing'])and!is_super_admin()){unset($config['licensing']);}return$config;},10,2);}static
function
handleAdmin(){add_action('admin_notices',function(){if(!is_super_admin())return;if(self::isUnauthorized()){$tf=self::isTF();$domain=self::domainUsed();if($tf&&$domain){$title=__('Purchase Code is already used','ait');$message=sprintf(__('Purchase Code %s is already used on "%s". If you want to use it on this website, you can %sderegister it here%s.','ait'),self::purchaseCode(),$domain,'<a href="https://system.ait-themes.club/account/themeforest">','</a>');}else{$title=!$tf?__('Invalid API Key for this domain','ait'):__('Invalid Purchase Code','ait');$message=!$tf?__('Please enter a valid API key for this domain. You can configure it in %sTheme Admin &rarr; Theme Options &rarr; API Key%s.','ait'):__('Please go to %sTheme Admin &rarr; Theme Options &rarr; ThemeForest Purchase Code%s to configure it.','ait');$message=sprintf($message,'<a href="'.admin_url('admin.php?page=ait-theme-options#ait-theme-options-licensing-panel').'">','</a>');}}else{$title=__('Error checking API Key','ait');$message=__('There was and error while checking API Key.','ait');}printf('<div class="notice notice-warning notice-large"><p><strong class="notice-title">%1$s</strong><br>%2$s</p></div>',esc_html($title),wp_kses_post($message));});}static
function
handleFrontend(){add_filter('template_include',function($template){if(self::isUnauthorized()){if(!self::isTF()){wp_die(__('Please enter a valid API key for this domain. You can configure it in Theme Options.','ait'),__('Invalid API Key for this domain','ait'));}else{if($domain=self::domainUsed()){wp_die(sprintf(__('This Purchase Code is already used on "%s". Please check WordPress admin for more details.','ait'),$domain),__('Purchase Code is already used','ait'));}else{wp_die(__('Please enter a valid ThemeForest Purchase Code. You can configure it in Theme Options.','ait'),__('Invalid Purchase Code','ait'));}}}return$template;});}protected
static
function
check(){if(self::isLocalhost()){return
200;}$current=get_transient('check_ait_subscription');if(!is_object($current)){$current=new
stdClass;}$newOption=new
stdClass;$newOption->lastChecked=time();$timeNotChanged=isset($current->lastChecked)&&(24*HOUR_IN_SECONDS)>(time()-$current->lastChecked);if($timeNotChanged){if(!empty($current->responseCode))return$current->responseCode;if(empty(self::apiKey())or
empty(self::purchaseCode())){return
401;}}$current->lastChecked=time();set_transient('check_ait_subscription',$current);$baseUrl=(defined('AIT_SYSTEM_API_TESTING_URL')and
AIT_SYSTEM_API_TESTING_URL)?AIT_SYSTEM_API_TESTING_URL:'https://system.ait-themes.club/api/5.0';$raw_response=wp_remote_post("$baseUrl/subscriptions/check",array('timeout'=>3,'body'=>array('domain'=>self::domain(),'key'=>self::isTF()?self::purchaseCode():self::apiKey(),'package'=>AIT_THEME_PACKAGE,'theme'=>AIT_THEME_CODENAME)));if(is_wp_error($raw_response)){error_log($raw_response->get_error_message());return
500;}$newOption->body=json_decode(wp_remote_retrieve_body($raw_response),true);$newOption->responseCode=wp_remote_retrieve_response_code($raw_response);set_transient('check_ait_subscription',$newOption);return$newOption->responseCode;}protected
static
function
isLocalhost(){$domain=self::domain();foreach(array('localhost','127.0.0.1')as$host){if(strpos($domain,$host)!==false){return
true;}}if(!empty($_SERVER['SERVER_PORT'])and!in_array($_SERVER['SERVER_PORT'],array(80,443))){return
true;}return
false;}static
function
isOk(){return
self::check()==200;}static
function
isUnauthorized(){return
self::check()==401;}static
function
isForbidden(){return
self::check()==403;}protected
static
function
isTF(){return(AIT_THEME_PACKAGE==='themeforest');}protected
static
function
domain(){$domain=preg_replace('|https?://|','',get_option('siteurl'));$slash=strpos($domain,'/');if($slash){$domain=substr($domain,0,$slash);}return$domain;}static
function
interceptSaveThemeOptions(){add_action('ait-save-options',function($data){if(!is_super_admin())return;if(isset($data['_ait_updater_options']['api_key'])){$options=get_option('_ait_updater_options',array());$options['api_key']=trim($data['_ait_updater_options']['api_key']);update_option('_ait_updater_options',$options);}elseif(isset($data['_ait_updater_options']['tf_purchase_code'])){$options=get_option('_ait_updater_options',array());$options['tf_purchase_code']=trim($data['_ait_updater_options']['tf_purchase_code']);update_option('_ait_updater_options',$options);}delete_transient('check_ait_subscription');});}}class
...
enqueueFrontendAssets(){$builtinAssets=apply_filters('ait-theme-builtin-assets',$this->builtinFrontendAssets);$this->enqueueCss($builtinAssets['css']);$this->enqueueJs($builtinAssets['js']);if(is_singular()and
comments_open()and
