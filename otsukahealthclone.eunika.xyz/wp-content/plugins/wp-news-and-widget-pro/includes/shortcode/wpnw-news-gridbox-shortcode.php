...
function wpnw_pro_get_gridbox_news( $atts, $content = null ) {

	// SiteOrigin Page Builder Gutenberg Block Tweak - Do not Display Preview
	if( isset( $_POST['action'] ) && ( $_POST['action'] == 'so_panels_layout_block_preview' || $_POST['action'] == 'so_panels_builder_content_json' ) ) {
		return '<div class="wpnw-pro-builder-shrt-prev">
					<div class="wpnw-pro-builder-shrt-title"><span>'.esc_html__('News Gridbox - Shortcode', 'sp-news-and-widget').'</span></div>
					[wpnw_gridbox]
				</div>';
	}
	...
	$shortcode_designs 				= wpnw_pro_gridbox_designs();
	$atts['content_tail'] 			= html_entity_decode( $atts['content_tail'] );
	$atts['limit']					= wpnw_pro_clean_number( $atts['limit'], 6, 'number' );
	...
	// Main Wrap
	$atts['css_clr']	= "{$design} {$extra_class}";
	...
	// WP Query
	$args 	= apply_filters( 'wpnw_pro_query_args', $args, $atts, 'wpnw_gridbox' );
	$args 	= apply_filters( 'wpnw_pro_news_gridbox_query_args', $args, $atts );
	...
	if ( $query->have_posts() ) {
		...
		wpnw_get_template( 'grid-box/loop-start.php', $atts ); // loop start 

		while ( $query->have_posts() ) : $query->the_post();

			$atts['post_link'] 				= wpnw_pro_get_post_link( $post->ID );
			$atts['post_featured_image'] 	= wpnw_pro_post_featured_image( $post->ID, 'large', true );
			$atts['news_post_title'] 		= get_the_title();
			$atts['cate_name'] 				= wpnw_pro_get_post_cats( $post->ID, $taxonomy, $link_target );

			wpnw_get_template( "grid-box/{$design}.php", $atts ); // loop designs

			$atts['grid_count']++;

		endwhile;

		wpnw_get_template( 'grid-box/loop-end.php', $atts ); // loop end
...
// 'wpnw_gridbox' shortcode
add_shortcode('wpnw_gridbox', 'wpnw_pro_get_gridbox_news');