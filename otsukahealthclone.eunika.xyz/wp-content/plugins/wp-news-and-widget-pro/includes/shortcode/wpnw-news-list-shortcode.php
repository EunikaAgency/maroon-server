...
function wpnw_pro_get_list_news( $atts, $content = null ) {

	// SiteOrigin Page Builder Gutenberg Block Tweak - Do not Display Preview
	if( isset( $_POST['action'] ) && ( $_POST['action'] == 'so_panels_layout_block_preview' || $_POST['action'] == 'so_panels_builder_content_json' ) ) {
		return '<div class="wpnw-pro-builder-shrt-prev">
					<div class="wpnw-pro-builder-shrt-title"><span>'.esc_html__('News List - Shortcode', 'sp-news-and-widget').'</span></div>
					[wpnw_news_list]
				</div>';
	}
	...
	$shortcode_designs 				= wpnw_pro_news_list_designs();
	...
	// WP Query
	$args 	= apply_filters( 'wpnw_pro_query_args', $args, $atts, 'wpnw_news_list' );
	$args 	= apply_filters( 'wpnw_pro_news_list_query_args', $args, $atts );
	...
	if ( $query->have_posts() ) {
		...
		wpnw_get_template( 'list/loop-start.php', $atts ); // Loop Start
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			wpnw_get_template( "list/{$design}.php", $atts ); // Loop Designs 

			$atts['newscount']++;
		endwhile;

		wpnw_get_template( 'list/loop-end.php', $atts ); // Loop End
...
// 'sp_news' shortcode
add_shortcode('wpnw_news_list', 'wpnw_pro_get_list_news');