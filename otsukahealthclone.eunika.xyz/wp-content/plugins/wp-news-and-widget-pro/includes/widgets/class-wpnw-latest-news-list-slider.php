<?php
/**
 * Widget API: Latest News List/Slider 1 Class
 *
...
class Wpnw_Pro_Lnlsw_Widget extends WP_Widget {
	...
	*/
	function __construct() {
		...
		$widget_ops = array('classname' => 'SP_Newslistpro_Widget', 'description' => __('Display Latest News Items in list view OR in Slider.', 'sp-news-and-widget'));
		parent::__construct( 'sp_newslistpro_widget', __('Latest News List/Slider 1 - Widget', 'sp-news-and-widget'), $widget_ops );

		// Widgets default values
		$this->defaults = array(
			'num_items'				=> 5,
			'title'					=> __( 'Latest News Slider/List-1', 'sp-news-and-widget' ),
			'date'					=> 1,
			'show_category'			=> 1,
	...
	function form( $instance ) {
		...
		if( class_exists( 'FusionBuilder' ) && ( isset( $_POST['action'] ) && $_POST['action'] == 'fusion_get_widget_form' ) ) { 
			$is_avada = 1;
		}
	?>
		<div class="wpnw-widget-wrap">
			...
			<!-- Active Slider -->
			<p>
				<h3><strong><?php esc_html_e( 'News Slider Setting', 'sp-news-and-widget' ); ?>:</strong></h3> 
				<hr>
				...
				<label for="<?php echo $this->get_field_id( 'active_slider' ); ?>"><?php _e( 'Check this box to Display News in Slider View.', 'sp-news-and-widget' ); ?></label> <br />
	...
	function widget( $news_args, $instance ) {
		...
		if( isset( $_POST['action'] ) && ( $_POST['action'] == 'so_panels_layout_block_preview' || $_POST['action'] == 'so_panels_builder_content_json' ) ) {
			echo '<div class="wpnw-pro-builder-shrt-prev">
						<div class="wpnw-pro-builder-shrt-title"><span>'.esc_html__('Latest News List/Slider 1 - Widget', 'sp-news-and-widget').'</span></div>
						Latest News List/Slider 1 Widget
					</div>';
			return;
		}
		...
		$atts['css_clr']	= ( $activeSlider == 1 )	? 'wpnw-has-slider'		: '';
		...
		// WP Query
		$news_args 		= apply_filters( 'wpnw_pro_wigets_query_args', $news_args, $atts, 'sp_newslistpro_widget' );
		$news_args 		= apply_filters( 'wpnw_pro_latest_news_list_slider_widgets_query_args', $news_args, $atts );
		...
		if ( $wpnw_query->have_posts() ) {
			...
			// Loop start
			wpnw_get_template( "widgets/news-list-slider/loop-start.php", $atts );
			...
			while ($wpnw_query->have_posts()) : $wpnw_query->the_post();
				...
				// Widget design
				wpnw_get_template( "widgets/news-list-slider/content.php", $atts );

			endwhile;

			// Loop end
			wpnw_get_template( "widgets/news-list-slider/loop-end.php", $atts );
