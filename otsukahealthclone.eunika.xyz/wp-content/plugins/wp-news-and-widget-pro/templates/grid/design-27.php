<?php
/**
 * Template for WP News and Scrolling Widgets Pro Loop - design-27
 *
 * This template can be overridden by copying it to yourtheme/wp-news-and-widget-pro/grid/design-27.php
 *
...
<?php } else { ?>
	...
	<div class="wpnaw-news-grid  wpnews-medium-<?php echo esc_attr($newsprogrid); ?> wpnews-columns wpnaw-news-smallimage <?php echo esc_attr($css_class); ?>" style="<?php echo esc_attr($height_css); ?>">
 		<div class="wpnaw-news-grid-content">
			<?php if ( $post_featured_image ) { ?>
				<div class="wpnaw-news-image-bg">
					<a href="<?php echo esc_url($post_link); ?>" target="<?php echo esc_attr($link_target) ?>">
						<img src="<?php echo esc_url($post_featured_image); ?>" alt="<?php echo wp_kses_post($news_post_title); ?>" />
					</a>
					<?php if($show_category_name && $cate_name !='') { ?>
						<div class="wpnaw-news-categories"><?php echo wp_kses_post($cate_name); ?></div>
					<?php } ?>
