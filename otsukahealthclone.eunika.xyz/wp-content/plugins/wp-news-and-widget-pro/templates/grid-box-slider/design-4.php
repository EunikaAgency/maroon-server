<?php
/**
 * Template for WP News and Five Widgets Pro Loop - design-4
 *
 * This template can be overridden by copying it to yourtheme/wp-news-and-widgets-pro/grid-box-slider/design-4.php
 *
...
$grid_box_count 	= ( $count % 5 );
$grid_box_count 	= ! empty( $grid_box_count ) 	? $grid_box_count : 5;
$image_height 		= (! empty($image_height)) 		? $image_height : '500';
$dynamic_height 	= (($count % 5) == 1) 			? $image_height : (($image_height/2)-2);
$height_css 		= ($dynamic_height) ? 'height:'.$dynamic_height.'px;' : '';

if( ( $count % 5 ) == 1 ) { ?>
<div class="wpnaw-grid-slider-wrp">
<?php } ?>
	<div class="wpnaw-news-slides wpnaw-clr-<?php echo esc_attr($grid_box_count); ?> wpnews-medium-4 wpnews-columns">
...
<?php if( ($count % 5) == 0 || ( $post_count == $count ) ) { ?>
</div>
<?php } ?>