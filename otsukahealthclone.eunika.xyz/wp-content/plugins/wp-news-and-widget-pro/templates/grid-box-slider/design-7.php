<?php
/**
 * Template for WP News and Five Widgets Pro Loop - design-6
 *
 * This template can be overridden by copying it to yourtheme/wp-news-and-widgets-pro/grid-box-slider/design-6.php
 *
...
$grid_box_count 	= ( $count % 4 );
$grid_box_count 	= ! empty( $grid_box_count ) 	? $grid_box_count : 4;
$image_height 		= (! empty($image_height)) 		? $image_height : '500';
$dynamic_height 	= (($count % 4) == 1) 			? $image_height : (($image_height/2)-2);
$height_css 		= ($dynamic_height) ? 'height:'.$dynamic_height.'px;' : '';

switch ($count % 4) {
	case '1':
		$dynamic_class = 'wpnews-medium-4';
		break;
	case '2':
		$dynamic_class = 'wpnews-medium-8';
		break;
	default:
		$dynamic_class = 'wpnews-medium-4';
		break;
...
<?php } ?>
	...
	<div class="wpnaw-news-slides wpnaw-clr-<?php echo esc_attr($grid_box_count); ?> <?php echo esc_attr($dynamic_class); ?> wpnews-columns">
...
<?php if( ($count % 4) == 0 || ( $post_count == $count ) ) { ?>
</div>
<?php } ?>
