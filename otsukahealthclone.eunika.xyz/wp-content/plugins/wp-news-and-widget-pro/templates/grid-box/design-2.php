<?php
/**
 * Template for WP News and Scrolling Widgets Pro Loop - design-2
 *
 * This template can be overridden by copying it to yourtheme/wp-news-and-widget-pro/grid-box/design-2.php
 *
...
if($grid_count == 1) {
	...
	$wpnw_post_large_image 	= wpnw_pro_post_featured_image( NULL, 'large', true ); // Post large image ?>
		...
		<?php if( ! empty( $wpnw_post_large_image ) ) { ?>
			<div class="wpnaw-news-image-bg" style="<?php echo esc_attr($height_css); ?>">
				<a href="<?php echo esc_url($post_link); ?>" target="<?php echo esc_attr($link_target); ?>">
					<img src="<?php echo esc_url($wpnw_post_large_image); ?>" alt="<?php the_title_attribute(); ?>" />
				</a>
			</div>
		<?php }
...
<?php } else { ?>
	...
	<div class="wpnews-medium-6 wpnews-columns flotRight">
		<div class="wpnaw-news-right-block">
