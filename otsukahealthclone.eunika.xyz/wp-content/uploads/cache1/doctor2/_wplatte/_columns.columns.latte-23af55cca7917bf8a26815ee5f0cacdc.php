...
?><?php
// prolog NCoreMacros
list($_l, $_g) = NCoreMacros::initRuntime($template, 'tgalap65yw')
;
// prolog NUIMacros
...
// main template
//
NCoreMacros::includeTemplate($element->common('header'), $template->getParameters(), $_l->templates['tgalap65yw'])->render() ?>
...
<?php if($columnElement->getId() === 'columns' or $columnElement->isEnabled()){
				NCoreMacros::includeTemplate($columnElement->getTemplate(), array('el' => $columnElement, 'element' => $columnElement, 'htmlId' => $columnElement->getHtmlId(), 'htmlClass' => $columnElement->getHtmlClass()) + $template->getParameters(), $_l->templates['tgalap65yw'])->render();
			}else{
