...
function wcdp_get_button_personalize($productID) {
	...
	if (!empty($dp_publish_page)) {
		...
		$link = do_shortcode(apply_filters('original_personalize_link', '<a href="' . $page_link . '" rel="nofollow" data-product_id="' . $productID . '" class="button product_type_customizable" style="display: none">' . __("Personalize", "wcdp") . '</a>', $page_link, $productID));
		return $link;
	}
}
