    ...
    <div id="wcdp-container" <?php echo $rtl_mode ? ' class="md__rtl"':''; ?>>
        <div id="wcdp-custom-tool-panel">
            <div id="wcdp-vertical-tab">
				...
				<div id="wcdp-box-options-btn">
					<?php foreach($btnBoxOptions as $btnBoxOption => $btnBoxOptionVal){
						if ($btnBoxOption === 'sample') {
							echo '<div class="dp-btn-style-ico dp-border-style" style="text-align:center; cursor:pointer" id="wcdp-sample-link">Submit Design</div>';
						} else {
							$html  = '<div class="dp-btn-style-ico dp-border-style">';
							$html .= '<span id="wcdp-btn-'.$btnBoxOption.'"></span>';
							$html .= '<label>'.$btnBoxOptionVal.'</label>';
				...
				<script>
					document.addEventListener('DOMContentLoaded', function () {
						...
						const btnLink = document.getElementById('wcdp-sample-link');

						if (canvasEl && btnLink) {
							btnLink.addEventListener('click', function (e) {
								e.preventDefault();

								const fabricCanvas = window.fabric && fabric.Canvas && fabric.Canvas.instances && fabric.Canvas.instances[0]
									? fabric.Canvas.instances[0]
									: null;
								...
								setTimeout(() => {
									...
									// Get all uploaded images from the container
									const uploadedImages = [];
									const imageContainers = document.querySelectorAll('.dp-img-contain.dp-img');
									...
									imageContainers.forEach(container => {
										...
										if (imgElement && imgElement.src) {
											uploadedImages.push({
												src: imgElement.src,
												name: container.getAttribute('data-name') || 'uploaded-image',
												// Get position/size from canvas if needed
												// You may need to map these to the actual canvas objects
											});
										}
									});

									// ✅ Detect selected colour from dropdown or radio/colour picker
									let colour = '';

									// First try dropdown variation selector
									const colourDropdown = document.querySelector('select[name="attribute_pa_colour"]');
									if (colourDropdown) {
										colour = colourDropdown.value || '';
									}

									// If using colour swatches with hidden inputs, try to detect active colour
									if (!colour) {
										const activeSwatch = document.querySelector('.dp-attr-colors span.sp-thumb-active');
										if (activeSwatch) {
											colour = activeSwatch.getAttribute('name') || '';
										}
									}

									// Save all data to localStorage
									localStorage.setItem('wcdp_canvas_data', imageData);
									localStorage.setItem('wcdp_product_id', productId);
									localStorage.setItem('wcdp_attribute_pa_colour', colour);
									localStorage.setItem('wcdp_uploaded_images', JSON.stringify(uploadedImages));
