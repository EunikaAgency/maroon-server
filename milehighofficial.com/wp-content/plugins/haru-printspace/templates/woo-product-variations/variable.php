...
<form class="variations_form cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo esc_attr( $variations_attr ); // WPCS: XSS ok. ?>">
    ...
    <div class="step-progress">
        <ul class="step-list" data-total-step="<?php echo esc_attr( $total_step ); ?>">
            <?php if ( $upload_enable == true ) : ?>
                <li class="step-active" data-step="1"><span><?php echo esc_html__( '01', 'haru-printspace' ); ?></span></li>
            <?php endif; ?>
            
            <?php foreach ( $attributes as $attribute_name => $options ) : ?>
                <li class="<?php echo ( $upload_enable == false && $i == 1 ) ? 'step-active' : ''; ?>" data-step="<?php echo esc_attr( $i ); ?>"><span><?php echo esc_html__( '0', 'haru-printspace' ); ?><?php echo esc_html( $i ); ?></span></li>
            <?php $i++; endforeach;?>

            <li data-step="<?php echo esc_attr( $total_step ); ?>"><span><?php echo esc_html__( '0', 'haru-printspace' ); ?><?php echo esc_html( $total_step ); ?></span></li>
        </ul>
    </div>

    <div class="variation-step-control" data-total-step="<?php echo esc_attr( $total_step ); ?>">
        <div class="variation-step-prev disable" data-prev=""><?php echo esc_html__( 'Back', 'haru-printspace' ); ?></div>
        <div class="variation-step-next disable" data-next="2"><?php echo esc_html__( 'Next Step', 'haru-printspace' ); ?></div>
    </div>

    <div class="variation-slide-step" data-step="1">
        <?php if ( $upload_enable == true ) : ?>
        <div class="variation-step"><span><?php echo esc_html__( 'Step 1', 'haru-printspace' ); ?></span></div>
        <?php endif; ?>
    ...
    <?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
        <p class="stock out-of-stock"><?php echo esc_html( apply_filters( 'woocommerce_out_of_stock_message', __( 'This product is currently out of stock and unavailable.', 'haru-printspace' ) ) ); ?></p>
    <?php else : ?>
            ...
            <?php foreach ( $attributes as $attribute_name => $options ) : ?>
                <div class="variation-slide-step slide-attribute" data-step="<?php echo esc_attr( $i ); ?>">
                    <div class="variation-row">
                        <div class="variation-step"><span><?php echo esc_html__( 'Step', 'haru-printspace' ); ?> <?php echo esc_html( $i ); ?></span></div>
                        <label for="<?php echo esc_attr( sanitize_title( $attribute_name ) ); ?>"><?php echo esc_html__( 'Choose your ', 'haru-printspace' ) . esc_html(wc_attribute_label( $attribute_name )); // WPCS: XSS ok. ?></label>
                        <div class="value">
                            <?php
                ...
                <?php if ( end( $attribute_keys ) === $attribute_name ): ?>
                    <div class="variation-row">
                        ...
                        <div class="value">
                            <?php echo wp_kses_post( apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html__( 'Clear selection', 'haru-printspace' ) . '</a>' ) ); ?>
                        </div>
                    </div>
                <?php endif; ?>
        ...
        <div class="variation-slide-step" data-step="<?php echo esc_attr( $i ); ?>">
            <?php if ( ! $product->is_sold_individually() ) : ?>
                <div class="add-to-cart-notice"><?php echo esc_html__( 'Choose Quantity', 'haru-printspace' ); ?></div>
            <?php else : ?>
                <div class="add-to-cart-notice"><?php echo esc_html__( 'Price Calculator', 'haru-printspace' ); ?></div>
            <?php endif; ?>

            <div class="single_variation_wrap">
                <div class="woocommerce-variation-price-notice"><?php echo esc_html__( 'The price will be visible here if configuration available.', 'haru-printspace' ); ?></div>
