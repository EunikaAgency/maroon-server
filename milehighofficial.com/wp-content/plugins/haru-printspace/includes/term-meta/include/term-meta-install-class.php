	...
	if ( ! class_exists( 'Haru_Term_Meta_Install' ) ):
		...
		final class Haru_Term_Meta_Install {
			...
			public function __construct() {
				...
				// Setup plugin
				$this->file     = HARU_PRINTSPACE_CORE_FILE;
				$this->url      = HARU_PRINTSPACE_CORE_URL;
				$this->path     = HARU_PRINTSPACE_CORE_DIR;
				$this->basename = HARU_PRINTSPACE_CORE_NAME;
				$this->db       = $wpdb;
			...
			public function terms_clauses( $pieces = array(), $taxonomies = array(), $args = array() ) {
				...
				if ( class_exists( 'RankMath' ) && isset( $args[ 'meta_query' ][0] ) && ( $args['meta_query'][0]['key'] == 'rank_math_robots' ) ) {
					return $pieces;
				}
