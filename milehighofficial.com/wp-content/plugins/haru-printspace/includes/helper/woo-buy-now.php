...
if ( ! class_exists( 'Haru_Woo_Buy_Now_Button' ) ) {
    class Haru_Woo_Buy_Now_Button {
        ...
        public function buy_now_button_single() {
            ...
            $button_text       = apply_filters( 'haru_woo_buy_now_button_text_single', esc_html__( 'Buy now', 'haru-printspace' ), $product_id );
            ...
            if ( ! empty( $custom_url ) && 'custom' === $redirect_location ) {
                // For custom link
                return printf( '<a href="%s" target="_blank" class="%s" rel="nofollow">%s</a>', esc_url( $custom_url ), esc_attr( $button_class ), esc_html__( $button_text, 'haru-printspace' ) );
            }

            return printf( '<button type="submit" name="haru-buy-now" value="%d" class="%s">%s</button>', $product_id, esc_attr( $button_class ), esc_html__( $button_text, 'haru-printspace' ) );
        }
        ...
        public function buy_now_button_archive() {
            ...
            $button_text       = apply_filters( 'haru_woo_buy_now_button_text_archive', esc_html__( 'Buy now', 'haru-printspace' ), $product_id );
            ...
            if ( $product->is_type( 'simple' ) ) {
                // For custom link
                if ( ! empty( $custom_url ) && 'custom' === $redirect_location ) {
                    return printf( '<a href="%s" target="_blank" data-quantity="%s" class="%s" data-product_id="%s" rel="nofollow">%s</a>', esc_url( $custom_url ), intval( $quantity ), esc_attr( $button_class ), $product_id, esc_html__( $button_text, 'haru-printspace' ) );
                }
                ...
                return printf( '<a href="%s" data-quantity="%s" class="%s" data-product_id="%s" rel="nofollow">%s</a>', esc_url( $redirect_url ), intval( $quantity ), esc_attr( $button_class ), $product_id, esc_html__( $button_text, 'haru-printspace' ) );
            }
        ...
        public function buy_now_button_submit() {
            ...
            if ( $product_id ) {
                if ( haru_get_option( 'haru_woo_buy_now_empty_cart', 'no' ) == '1' ) {
                    WC()->cart->empty_cart();
                }
