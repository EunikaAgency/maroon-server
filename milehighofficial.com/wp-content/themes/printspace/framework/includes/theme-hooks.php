  ...
  function haru_login_popup() {
    ...
    <div id="login-popup" class="login-modal unero-modal woocommerce-account" tabindex="-1" role="dialog">
      ...
      <div class="modal-footer">
        <a href="#" class="close-modal"><?php echo esc_html__( 'Close', 'printspace' ) ?></a>
      </div>
    </div>
    <?php
  }
...
if ( ! function_exists( 'haru_link_pages' ) ) {
  function haru_link_pages() {
    wp_link_pages( array(
      'before'      => '<div class="haru-page-links"><span class="haru-page-links-title">' . esc_html__( 'Pages:', 'printspace' ) . '</span>',
      'after'       => '</div>',
    ...
    <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
      <div id="comment-<?php comment_ID(); ?>" class="comment-body">
        ...
        <div class="comment-text">
          <div class="author">
            <div class="author-name"><?php printf( esc_html__( '%s', 'printspace' ), get_comment_author_link() ) ?></div>
          </div>
          <div class="text">
            <?php comment_text(); ?>
            <?php if ( $comment->comment_approved == '0' ) : ?>
              <em><?php echo esc_html__( 'Your comment is awaiting moderation.', 'printspace' ) ?></em>
            <?php endif; ?>
          </div>
          <div class="comment-meta">
            ...
            <div class="comment-meta-action">
              ...
              <?php edit_comment_link( esc_html__( 'Edit', 'printspace' ), '', '' ); ?>
            </div>
          </div>
        </div>
      </div>
  ...
  ?>
    <div id="respond-wrapper">
      <?php
        ...
        $fields    =  array(
          'author'        => '<div class="haru-row haru-field-group"><div class="comment-form-author haru-field haru-col-sm-6 haru-col-xs-12"><input id="author" name="author" type="text" placeholder="' . esc_attr__( 'Enter Your Name*', 'printspace' ) . '" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></div>',
          'email'         => '<div class="comment-form-email haru-field haru-col-sm-6 haru-col-xs-12"><input id="email" name="email" type="text" placeholder="' . esc_attr__( 'Enter Your Email*', 'printspace' ) . '" class="form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></div></div>',
          'comment_field' => '<div class="haru-row haru-field-group"><div class="haru-col-sm-12"><div class="comment-form-comment haru-field"><textarea class="form-control" placeholder="' . esc_attr__( 'Enter Your Comment', 'printspace' ) . '" id="comment" name="comment" cols="40" rows="6" aria-required="true"></textarea></div></div></div>'
        );
        ...
        $comments_args = array(
            ...
            sprintf(
            __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s">Log out?</a>', 'printspace' ),
              admin_url( 'profile.php' ),
              $user_identity,
              wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) )
            ) . '</p>',
            'title_reply'          => esc_html__( 'Leave us a comment', 'printspace' ),
            'title_reply_to'       => esc_html__( 'Leave a reply to %s', 'printspace' ),
            'title_reply_before'   => '<h3 id="reply-title" class="comment-reply-title haru-heading haru-heading--style-1">',
            'cancel_reply_link'    => esc_html__( 'Cancel Reply', 'printspace' ),
            ...
            'label_submit'         => esc_html__( 'Post Comment', 'printspace' ),
            'comment_field'        => '',
            'must_log_in'          => ''
        );

        if ( is_user_logged_in() ) {
          $comments_args['comment_field'] = '<p class="comment-form-comment"><textarea class="form-control" placeholder="' . esc_attr__( 'Enter Your Comment', 'printspace' ) . '" id="comment" name="comment" cols="40" rows="6" aria-required="true"></textarea></p>';
        }
      ?>
      <?php if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) : ?>
        <?php echo sprintf(
            __( '<div class="comment-require-login">You must be <a href="%1$s">logged in</a> to post a comment!</div>', 'printspace' ),
