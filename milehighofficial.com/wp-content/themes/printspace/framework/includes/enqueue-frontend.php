...
if ( ! function_exists( 'haru_fonts_url' ) ) {
    function haru_fonts_url() {
        ...
        $jost = _x( 'on', 'Jost font: on or off', 'printspace' );
        $smooch = _x( 'on', 'Smooch font: on or off', 'printspace' );

        if ( 'off' !== $jost || 'off' !== $smooch ) {
            $font_families = array();
        }

        if ( 'off' !== $jost ) {
            $font_families[] = 'Jost:100,200,300,400,500,600,700,800,900,100italic,200italic,300italic,400italic,500italic,600italic,700italic,800italic,900italic';
        }
...
if ( ! function_exists( 'haru_enqueue_script' ) ) {
    function haru_enqueue_script() {
        ...
        if ( class_exists( 'Haru_PrintSpace' ) ) {
            wp_add_inline_script( 'haru-theme-script', 'var haru_printspace_ajax_url = "' . get_site_url() . '/wp-admin/admin-ajax.php?activate-multi=true' . '"', 'before' );
        }
    }
