...
class Preload
{
    ...
    public static function add_critical_image_preloads(&$html, $clean_html) {
        ...
        if(!empty($parent_exclusions)) {
            ...
            $clean_dom->loadHTML(htmlentities($clean_html), LIBXML_SCHEMA_CREATE | LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_NOERROR | LIBXML_ERR_NONE);
            ...
            if($image_excluded) {
                ...
                $dom = new \DOMDocument();
                $dom->encoding = 'utf-8';
                $dom->loadHTML(htmlentities($html), LIBXML_SCHEMA_CREATE | LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_NOERROR | LIBXML_ERR_NONE);
                $html = $dom->saveHTML();
