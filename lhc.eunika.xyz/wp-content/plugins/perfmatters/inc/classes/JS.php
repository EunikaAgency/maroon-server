		...
		foreach($matches[0] as $i => $tag) {
			...
			if($defer_check) {
				...
				if(empty($atts_array['src'])) {
					...
					//option not set
					if(empty(Config::$options['assets']['defer_inline'])) {
						continue;
					}

					//exclusion check
					if(!empty($js_exclusions) && preg_match('#(' . $js_exclusions . ')#i', $tag)) {
						continue;
					}

					//add defer to opening tag
					$deferred_tag_open = str_replace('>', ' defer src="data:text/javascript;base64,' . base64_encode($matches[3][$i]) . '">', $matches[1][$i]);

					$deferred_tag = $deferred_tag_open . '</script>'; 
				...
				//standard script
					...
					if(isset($atts_array['async'])) {
						continue;
					}

					//deal with wp strategy data attribute
					if(!empty($atts_array['data-wp-strategy']) && $atts_array['data-wp-strategy'] == 'async') {
						continue;
					}

					//skip if there is already a defer
					if(isset($atts_array['defer'])) {
						continue;
					}
