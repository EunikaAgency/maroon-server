...
require_once( HARU_PRINTSPACE_CORE_DIR . 'includes/scss/scssphp/scss.inc.php' );
...
if ( ! defined( 'HARU_DEVELOPE_MODE' ) && ( '1' == haru_get_option( 'haru_scss_compiler', '0' ) ) ) {
    add_action( 'redux/options/haru_printspace_options/saved', 'haru_generate_scss' );
}
...
if ( ! function_exists( 'haru_generate_scss' ) ) {
    function haru_generate_scss() {
        try {
            ...
            $scss->addVariables( $color_variables );
            ...
            if ( !$wp_filesystem->put_contents( $file_path, $css, FS_CHMOD_FILE) ) {
                return array(
                    'status'  => 'error',
                    'message' => esc_html__( 'Could not save file! Please check your Server Permissions.', 'haru-printspace' )
                );
            }
        } catch( Exception $e ) {
...
if ( ! function_exists( 'haru_theme_color_scss_variables' ) ) {
    function haru_theme_color_scss_variables() {
        ...
        $gradient_color_5 = haru_get_option( 'haru_gradient_color_5', '#ffd0b1' );
        $gradient_color_6 = haru_get_option( 'haru_gradient_color_6', '#ffd0b1' );
        $gradient_color_7 = haru_get_option( 'haru_gradient_color_7', '#ffd0b1' );
        ...
        $scss_variables['cl-gr-5']            = $gradient_color_5;
        $scss_variables['cl-gr-6']            = $gradient_color_6;
        $scss_variables['cl-gr-7']            = $gradient_color_7;
        ...
        if ( is_multisite() ) {
            if ( is_main_site() ) {
                $scss_variables['theme-url']             = '../themes/printspace/';
            } else {
                $scss_variables['theme-url']             = '../../../themes/printspace/';
            }
        } else {
