...
<?php if ( $settings['list'] ) : ?>
			...
			<li class="haru-images-gallery__item haru-images-gallery__hover-<?php echo esc_attr( $settings['hover'] ); ?>">
				<div class="haru-images-gallery__image">
					<a href="<?php echo esc_url( $item['list_image']['url'] ); ?>" class="gallery-popup-link" data-elementor-open-lightbox="no">
						...
						<div class="haru-gallery__overlay">
							<div class="haru-gallery__overlay-content">
								<?php if ( ! empty( $item['list_title'] ) ) : ?>
									<p class="haru-gallery__title"><?php echo esc_html( $item['list_title'] ); ?></p>
								<?php endif; ?>
								<?php if ( ! empty( $item['list_description'] ) ) : ?>
									<p class="haru-gallery__description"><?php echo esc_html( $item['list_description'] ); ?></p>
								<?php endif; ?>
							</div>
						</div>
					</a>
	...
	<ul class="haru-images-gallery__list haru-slick haru-slick--nav-opacity haru-slick--nav-center" data-slick='{"slidesToShow" : <?php echo esc_attr( $settings['slidesToShow'] ); ?>, "slidesToScroll" : <?php echo esc_attr( $settings['slidesToScroll'] ); ?>, "arrows" : <?php echo esc_attr( ( 'yes' == $settings['arrows'] ) ? 'true' : 'false' ); ?>, "infinite" : true, "focusOnSelect" : true, "vertical" : false, "autoplay": <?php echo esc_attr( ( 'yes' == $settings['autoPlay'] ) ? 'true' : 'false' ); ?>, "autoplaySpeed": <?php echo esc_attr( $settings['autoPlaySpeed'] ? $settings['autoPlaySpeed'] : '3000' ); ?>, "responsive" : [{"breakpoint" : 991,"settings" : {"slidesToShow" : <?php echo esc_attr( $settings['slidesToShow_tablet'] ); ?>, "slidesToScroll" : <?php echo esc_attr( $settings['slidesToScroll_tablet'] ); ?>}}, {"breakpoint" : 767,"settings" : {"slidesToShow" : <?php echo esc_attr( $settings['slidesToShow_mobile'] ); ?>, "slidesToScroll" : <?php echo esc_attr( $settings['slidesToScroll_mobile'] ); ?>}}] }'>
		<?php 
			foreach (  $settings['list'] as $item ) :
		?>
			<li class="haru-images-gallery__item haru-images-gallery__hover-<?php echo esc_attr( $settings['hover'] ); ?>">
    ...
    <?php elseif ( in_array( $settings['pre_style'], array( 'scroll' ) ) ) : ?>
  		...
  		<?php
	      	...
	      	foreach ( $settings['list'] as $item ) : 
      	?>
  		<div class="haru-images-gallery__item-wrap haru-images-gallery__columns-<?php echo esc_attr( $settings['scroll_columns'] ); ?> haru-images-gallery__columns--tablet-<?php echo esc_attr( $settings['scroll_columns_tablet'] ); ?> haru-images-gallery__columns--mobile-<?php echo esc_attr( $settings['scroll_columns_mobile'] ); ?>">
