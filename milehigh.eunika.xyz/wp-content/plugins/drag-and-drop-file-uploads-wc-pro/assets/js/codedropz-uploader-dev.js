		...
		this.each(function(){
			...
			var CodeDropzUploader = {
				...
				abortFile : function( file ) {
                    var WC_storageType = dnd_wc_uploader.storage_type;
					if( FileOptions.chunking && WC_storageType == 'local_storage' ) {
						if( file.chunkTransfer )
							file.chunkTransfer.abort();
					}else if( WC_storageType == 'remote_storage' ){
                        if( file.chunkTransfer )
							file.chunkTransfer.abort();
                    }else{
                        file.transfer.abort();
				...
				uploadFile : function( data, file, name ) {
                    ...
                    var storageTypes = dnd_wc_uploader.storage_type;

					// Setup form data
					//formData.append('supported_type', options.supported_type ); //@note  : removed due to security reason
					...
					// Chunks file upload
					if( FileOptions.chunking && file.size > chunkSize && storageTypes == 'local_storage' ) {
						file.queued = true;
						file.chunkSize = chunkSize;
						file.totalChunks = Math.ceil( file.size / file.chunkSize );
						file.currentChunk = 0;
                        file.action = 'dnd_codedropz_wc_upload_chunks';
						this.uploadChunks( data, file, name );
                    }else if( storageTypes == 'remote_storage' ) {
                        
                        // Setup chunks file data
						file.chunkSize = parseInt( dnd_wc_uploader.storage_chunks ); // formData - this cause the issue not adding headers in ajax request format: 1000*1024
                        file.action = 'codedropz_storage_chunks_upload-wc';
                        
                        file.queued = true;
                        file.currentChunk = 0;

                        // Adjust chunk size for amazon (default chunks - 5MB)
                        if( dnd_wc_uploader.storage_name == 'amazon' ) {
                            file.chunkSize = 5242880; //5MB
                        }
				...
				uploadChunks : function( data, file, name ){
                    ...
                    if( file.data ) {
                        var file_data = JSON.stringify( file.data );
                        if( formData.has('data') ) {
                            formData.set('data', file_data );
                        }else{
                            formData.append('data', file_data );
                        }
                    }
