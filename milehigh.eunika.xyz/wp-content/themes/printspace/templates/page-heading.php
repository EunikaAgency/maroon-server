...
if ( is_home() ) {
    ...
    if ( $show_on_front == 'posts' ) {
        $haru_page_title = esc_html__( 'Blog', 'printspace' );
...
if ( is_category() || is_tag() || is_author() || is_day() || is_month() || is_year() || is_search() || is_tax( 'post_format' ) ) {
    ...
    if ( is_category() ) {
        ...
        $haru_page_title = single_tag_title( esc_html__( 'Tags: ', 'printspace' ), false );
    } elseif ( is_author() ) {
        $haru_page_title = sprintf( esc_html__( 'Author: %s', 'printspace' ), get_the_author() );
    } elseif ( is_day() ) {
        $haru_page_title = sprintf( esc_html__( 'Daily Archives: %s', 'printspace' ), get_the_date() );
    } elseif ( is_month() ) {
        $haru_page_title = sprintf( esc_html__( 'Monthly Archives: %s', 'printspace' ), get_the_date(_x('F Y', 'monthly archives date format', 'printspace' ) ) );
    } elseif ( is_year() ) {
        $haru_page_title = sprintf( esc_html__( 'Yearly Archives: %s', 'printspace' ), get_the_date(_x('Y', 'yearly archives date format', 'printspace' ) ) );
    } elseif ( is_search() ) {
        $haru_page_title = sprintf( esc_html__( 'Search Results for: %s', 'printspace' ), get_search_query() );
    } elseif ( is_tax( 'post_format', 'post-format-aside' ) ) {
        $haru_page_title = esc_html__( 'Asides', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-gallery' )) {
        $haru_page_title = esc_html__( 'Galleries', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-image')) {
        $haru_page_title = esc_html__( 'Images', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-video' )) {
        $haru_page_title = esc_html__( 'Videos', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-quote' )) {
        $haru_page_title = esc_html__( 'Quotes', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-link' )) {
        $haru_page_title = esc_html__( 'Links', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-status' )) {
        $haru_page_title = esc_html__( 'Statuses', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-audio' )) {
        $haru_page_title = esc_html__( 'Audios', 'printspace' );
    } elseif ( is_tax( 'post_format', 'post-format-chat' )) {
        $haru_page_title = esc_html__( 'Chats', 'printspace' );
    }
}
...
if ( is_404() ) {
    $haru_page_title = esc_html__( 'Nothing Found', 'printspace' );
}
        ...
        <div class="haru-page-title__content <?php echo esc_attr( $haru_title_content_layout ); ?>">
            ...
            <div class="haru-page-title__heading">
                <?php if ( isset( $haru_page_title ) && ( $haru_page_title != '' ) ) : ?>
                    <h1 class="haru-page-title__heading--main"><?php echo esc_html( $haru_page_title ); ?></h1>
                <?php endif; ?>
