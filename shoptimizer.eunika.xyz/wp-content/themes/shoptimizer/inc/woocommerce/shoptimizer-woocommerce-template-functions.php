...
if ( ! function_exists( 'shoptimizer_cart_link_shortcode' ) ) {
	...
	function shoptimizer_cart_link_shortcode() {
		...
		if ( true === $shoptimizer_layout_woocommerce_enable_sidebar_cart ) {
			$html .= '<a class="cart-contents" href="#" title="' . esc_attr__( 'View your shopping cart', 'shoptimizer' ) . '">';
		} else {
			$html .= '<a class="cart-contents" href="' . wc_get_cart_url() . '" title="' . esc_attr__( 'View your shopping cart', 'shoptimizer' ) . '">';
		}
	...
	?>
		<?php if ( true === $shoptimizer_layout_woocommerce_prev_next_display ) { ?>
			...
			<div class="shoptimizer-product-prevnext">
			    ...
			    $shoptimizer_prev = get_adjacent_in_stock_product(true, '', true, 'product_cat');
			    $shoptimizer_next = get_adjacent_in_stock_product(true, '', false, 'product_cat');
			    ?>
			    <?php if ($shoptimizer_prev) : ?>
			        <a href="<?php echo esc_url($shoptimizer_prev->get_permalink()); ?>" aria-label="<?php echo shoptimizer_safe_html($shoptimizer_prev->get_name()); ?>">
			            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 15l-3-3m0 0l3-3m-3 3h8M3 12a9 9 0 1118 0 9 9 0 01-18 0z" />
			            </svg>
			            <div class="tooltip">
			                <?php echo shoptimizer_safe_html($shoptimizer_prev->get_image()); ?>
			                <span class="title"><?php echo shoptimizer_safe_html($shoptimizer_prev->get_name()); ?></span>
			                <span class="prevnext_price"><?php echo shoptimizer_safe_html($shoptimizer_prev->get_price_html()); ?></span>								
			            </div>
			        </a>
			    <?php endif; ?>
			    <?php if ($shoptimizer_next) : ?>
			        <a href="<?php echo esc_url($shoptimizer_next->get_permalink()); ?>" aria-label="<?php echo shoptimizer_safe_html($shoptimizer_next->get_name()); ?>">
			            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 9l3 3m0 0l-3 3m3-3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z" />
			            </svg>
			            <div class="tooltip">
			                <?php echo shoptimizer_safe_html($shoptimizer_next->get_image()); ?>
			                <span class="title"><?php echo shoptimizer_safe_html($shoptimizer_next->get_name()); ?></span>
	...
	function shoptimizer_cart_progress() {
		...
		if ( true === $shoptimizer_layout_progress_bar_display ) {
			...
			<ul class="checkout-bar">
				...
				<a href="<?php echo get_permalink( wc_get_page_id( 'cart' ) ); ?>"><?php esc_html_e( 'Quote List', 'shoptimizer' ); ?></a></span>
				</li>
				<li class="next">
				<span>
				<a href="<?php echo get_permalink( wc_get_page_id( 'checkout' ) ); ?>"><?php esc_html_e( 'Finalise Request', 'shoptimizer' ); ?></a></span></li>
	...
	if ( true === $shoptimizer_cross_sells_carousel ) {
	    ...
	    if ( $shoptimizer_cross_sells ) {
		    ...
		    foreach ( $shoptimizer_cross_sells as $cross_sell_id ) {
		        $shoptimizer_cross_sell_product 	= wc_get_product( $cross_sell_id );
		        $shoptimizer_cross_sell_link 		= get_permalink( $cross_sell_id );
		        $shoptimizer_cross_sell_title 		= $shoptimizer_cross_sell_product->get_name();
		        $shoptimizer_cross_sell_img 		= $shoptimizer_cross_sell_product->get_image( 'woocommerce_thumbnail' );
		        $shoptimizer_cross_sell_price 		= $shoptimizer_cross_sell_product->get_price_html();
		        ...
		        // Add to cart button
			    if ( $shoptimizer_cross_sell_product->is_type( 'simple' ) ) {
			        ...
			        echo sprintf( '<a href="%s" data-quantity="1" class="product_type_simple add_to_cart_button ajax_add_to_cart" data-product_id="%d" data-product_sku="%s" rel="nofollow">%s</a>',
			            esc_url( $shoptimizer_cross_sell_product->add_to_cart_url() ),
			            esc_attr( $shoptimizer_cross_sell_product->get_id() ),
			            esc_attr( $shoptimizer_cross_sell_product->get_sku() ),
			            esc_html( $shoptimizer_cross_sell_product->add_to_cart_text() )
			        );
			        echo '</div>';
			    } else {
			        echo '<div class="pdp-complementary--add-to-cart">';
			        echo sprintf( '<a href="%s" data-quantity="1" class="product_type_%s" data-product_id="%d" data-product_sku="%s" rel="nofollow">%s</a>',
			            esc_url( $shoptimizer_cross_sell_product->add_to_cart_url() ),
			            esc_attr( $shoptimizer_cross_sell_product->get_type() ),
			            esc_attr( $shoptimizer_cross_sell_product->get_id() ),
			            esc_attr( $shoptimizer_cross_sell_product->get_sku() ),
			            esc_html( $shoptimizer_cross_sell_product->add_to_cart_text() )
			        );
