...
if ( ! function_exists( 'shoptimizer_myaccount_icon' ) ) {
	/**
	 * Display an account icon in the header.
	 *
	 * @since  1.0.0
	 * @return void
	 */
...
if ( ! function_exists( 'shoptimizer_mobile_menu_close' ) ) {
	...
	function shoptimizer_mobile_menu_close() {
		echo '<button class="mobile-menu close-drawer" aria-label="Close menu">
...
function shoptimizer_sidebar_body_class( $classes ) {
	...
	// Check if we are on the blog, blog category, or blog tag archives
    if ( is_home() || is_category() || is_tag() || is_author() || is_date() ) {
        // Add the retrieved sidebar layout option to the body classes if it's not empty
        if ( !empty( $shoptimizer_layout_archives_sidebar ) ) {
            $classes[] = sanitize_html_class( $shoptimizer_layout_archives_sidebar );
        }
    }
...
function shoptimizer_page_sidebar_body_class($classes) {
    ...
    if (is_page() && basename(get_page_template()) == 'page.php') {
        if (!empty($shoptimizer_layout_page_sidebar)) {
            $classes[] = sanitize_html_class($shoptimizer_layout_page_sidebar);
        }
    }
...
/**
 * Remove prefix before blog archive titles.
 */
	...
	function shoptimizer_search_modal() {
		...
		<dialog class="shoptimizer-modal" data-shoptimizermodal-id="searchToggle" aria-label="Display search modal">
            <div class="shoptimizer-modal--container">
	            <form method="dialog">
	            	<div role="heading"><?php echo shoptimizer_safe_html( $shoptimizer_layout_search_title ); ?></div>
...
/**
 * Remove shortcodes from excerpts, e.g. search results pages.
 */

/*
function shoptimizer_strip_shortcode_from_excerpt( $content ) {
    $content = strip_shortcodes( $content );
}
add_filter('the_excerpt', 'shoptimizer_strip_shortcode_from_excerpt');
*/
