...
function shoptimizer_scripts() {
	...
	if (shoptimizer_is_woocommerce_activated()) {
		if ('enable' === $shoptimizer_wc_product_category_widget_toggle) {
			wp_enqueue_style('shoptimizer-wc-product-categories-toggle', get_template_directory_uri() . '/assets/css/components/widgets/wc-product-categories-toggle.css', '', $shoptimizer_version);
		}
	}
	...
	if (shoptimizer_is_woocommerce_activated()) {
		if (true === $shoptimizer_cross_sells_carousel) {
			wp_enqueue_style('shoptimizer-cross-sells-carousel', get_template_directory_uri() . '/assets/css/components/pdp/cross-sells-carousel.css', '', $shoptimizer_version);
		}
	}
	...
	if (shoptimizer_is_woocommerce_activated()) {
		if (true === $shoptimizer_layout_woocommerce_sticky_cart_display) {
			if (is_product()) {
				wp_enqueue_style('shoptimizer-pdp-legacy-sticky-atc', get_template_directory_uri() . '/assets/css/components/pdp/legacy-sticky-atc.css', '', $shoptimizer_version);
			}
		}
	}
}
...
function shoptimizer_register_required_plugins() {
	$plugins = array(
		...
		array(
			...
			'source'   => 'https://files.commercegurus.com/commercekit/2.3.9/commercegurus-commercekit.zip',
			'required' => false,
			'version'  => '2.3.9',
		),
		array(
...
add_action('after_setup_theme', function () {
	add_image_size('card-thumb', 392, 543, true); // exact display size
});


function my_responsive_image($image_id, $size = 'medium') {
...
add_action('wp_head', function () {
	if (is_front_page()) {
		$id = 7196; // LCP image ID
		$src = wp_get_attachment_image_url($id, 'card-thumb');
		$srcset = wp_get_attachment_image_srcset($id, 'card-thumb');
?>
		<link rel="preload"
			as="image"
			href="<?php echo esc_url($src); ?>"
			imagesrcset="<?php echo esc_attr($srcset); ?>"
			imagesizes="(max-width: 550px) 100vw, 392px">
	<?php
	}
});
...
// Add custom button on edit product page
add_action('post_submitbox_misc_actions', 'wc_custom_button_edit_product');
function wc_custom_button_edit_product() {
	global $post;
	...
	<div class="misc-pub-section" style="padding-top:10px;">
		<a href="<?php echo admin_url('admin.php?page=my-custom-page&product_id=' . $post->ID); ?>"
...
// Register the shortcode
add_shortcode('category_below_content', 'category_below_content_shortcode');

function category_below_content_shortcode() {
	if (!is_category() && !is_tax()) {
		return ''; // only works on category or taxonomy archive pages
	}
	...
	// Get only the "below_category_content" meta
	$content = get_term_meta($term->term_id, 'below_category_content', true);

	if (empty($content)) {
		return ''; // nothing saved for this category
	}

	// Convert line breaks into proper <p> tags
	$content = wpautop($content);

	// Output wrapped in Elementor container
	return '<div class="elementor-element elementor-element-b4f26a9 e-flex e-con-boxed e-con e-parent e-lazyloaded" data-id="b4f26a9" data-element_type="container">
                <div class="e-con-inner">
                    <div class="elementor-element elementor-element-a3a15d8 elementor-widget elementor-widget-woocommerce-archive-description" data-id="a3a15d8" data-element_type="widget" data-widget_type="woocommerce-archive-description.default">
                        <div class="elementor-widget-container">
                            <div class="term-description">' . wp_kses_post($content) . '</div>
                        </div>
                    </div>
                </div>
            </div>';
...
// Add custom fields on add form
add_action('pa_colour_add_form_fields', function () {
?>
	<div class="form-field">
		<label for="term_color">Color</label>
		<input type="color" name="term_color" id="term_color" value="">
	</div>

	<div class="form-field">
		<label for="is_dual_color">
			<input type="checkbox" name="is_dual_color" id="is_dual_color" value="1"> Is Dual
		</label>
	</div>

	<div class="form-field" id="term_color_2_wrapper" style="display:none;">
		<label for="term_color_2">Secondary Color</label>
		<input type="color" name="term_color_2" id="term_color_2" value="">
	</div>

	<script>
		document.addEventListener("DOMContentLoaded", function() {
			const checkbox = document.getElementById('is_dual_color');
			const secondary = document.getElementById('term_color_2_wrapper');
			checkbox.addEventListener('change', function() {
				secondary.style.display = this.checked ? 'block' : 'none';
			});
		});
	</script>
<?php
...
// Add custom fields on edit form
add_action('pa_colour_edit_form_fields', function ($term) {
	$color = get_term_meta($term->term_id, 'term_color', true);
	$is_dual_color = get_term_meta($term->term_id, 'is_dual_color', true);
	$secondary = get_term_meta($term->term_id, 'term_color_2', true);
?>
	<tr class="form-field">
		<th scope="row"><label for="term_color">Color</label></th>
		<td>
	...
	<tr class="form-field">
		<th scope="row"><label for="is_dual_color">Is Dual</label></th>
		<td>
			<input type="checkbox" name="is_dual_color" id="is_dual_color" value="1" <?php checked($is_dual_color, 1); ?>>
		</td>
	</tr>

	<tr class="form-field" id="term_color_2_wrapper" style="<?php echo $is_dual_color ? '' : 'display:none;'; ?>">
		<th scope="row"><label for="term_color_2">Secondary Color</label></th>
		<td>
			<input type="color" name="term_color_2" id="term_color_2" value="<?php echo esc_attr($secondary); ?>">
		</td>
	</tr>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function() {
			...
			const secondary = document.getElementById('term_color_2_wrapper');
			checkbox.addEventListener('change', function() {
				secondary.style.display = this.checked ? '' : 'none';
			});
		});
	</script>
<?php
...
// Save fields
add_action('created_pa_colour', 'save_pa_colour_fields');
add_action('edited_pa_colour', 'save_pa_colour_fields');
function save_pa_colour_fields($term_id) {
	if (isset($_POST['term_color'])) {
		update_term_meta($term_id, 'term_color', sanitize_hex_color($_POST['term_color']));
	}
	update_term_meta($term_id, 'is_dual_color', isset($_POST['is_dual_color']) ? 1 : 0);
	if (isset($_POST['term_color_2'])) {
		update_term_meta($term_id, 'term_color_2', sanitize_hex_color($_POST['term_color_2']));
	}
}
...
// 	$shortcode_file = get_template_directory() . '/shortcodes/related-products.php';
// 	$shortcode_file = get_template_directory() . '/shortcodes/search.php';
...
function register_custom_shortcodes() {
	$related_shortcode_file = get_template_directory() . '/shortcodes/related-products.php';
	$search_shortcode_file  = get_template_directory() . '/shortcodes/search.php';

	if (file_exists($related_shortcode_file)) {
		require_once $related_shortcode_file;
		add_shortcode('related_products', 'related_products_shortcode');
	}

	if (file_exists($search_shortcode_file)) {
		require_once $search_shortcode_file;
		add_shortcode('threads_search', 'threads_search_shortcode');
	}
}
add_action('init', 'register_custom_shortcodes');
