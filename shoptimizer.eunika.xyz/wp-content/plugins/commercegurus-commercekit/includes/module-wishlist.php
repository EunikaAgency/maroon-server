...
function commercekit_shortcode_wishlist() {
	...
	<table class="commercekit-wishlist-table" width="100%">
		<thead>
			<tr>
				<th id="product" colspan="3"><?php esc_html_e( 'Product', 'commercegurus-commercekit' ); ?></th>
			...
			<tr id="product-<?php echo esc_attr( $row['product_id'] ); ?>">
				...
				<td class="cart">
					<?php if ( $in_stock && ( $product->is_type( 'simple' ) || $product->is_type( 'variable' ) ) ) { ?>
						...
						<?php } else { ?>
							<a class="button" href="<?php echo esc_url( $product->get_permalink() ); ?>"><?php esc_html_e( 'Select Options', 'commercegurus-commercekit' ); ?></a>
						<?php } ?>
					<?php } else { ?>
						<a class="button" href="<?php echo esc_url( $product->get_permalink() ); ?>"><?php esc_html_e( 'View Product', 'commercegurus-commercekit' ); ?></a>
					<?php } ?>
				</td>
			</tr>
					<?php
...
function commercekit_ajax_wishlist_update() {
	...
	if ( count( $wlist_ids ) && ( $uid || $sid ) ) {
		...
		if ( $uid ) {
			$wids = $wpdb->get_col( $wpdb->prepare( 'SELECT product_id FROM ' . $table_item . ' WHERE user_id = %d AND product_id IN (' . $wlist_txt . ')', $uid ) ); // phpcs:ignore
		} else {
			$wids = $wpdb->get_col( $wpdb->prepare( 'SELECT product_id FROM ' . $table_item . ' WHERE list_id = %d AND product_id IN (' . $wlist_txt . ')', $sid ) ); // phpcs:ignore
		}
