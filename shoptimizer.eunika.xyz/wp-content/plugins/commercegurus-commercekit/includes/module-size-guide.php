...
function commercekit_sg_admin_tags_display( $post ) {
	$options = '';
	if ( isset( $post->ID ) && $post->ID ) {
		$tags = get_post_meta( $post->ID, 'commercekit_sg_tag', true );
		$tags = explode( ',', $tags );
...
function commercekit_sg_admin_attributes_display( $post ) {
	$options = '';
	if ( isset( $post->ID ) && $post->ID ) {
		$attributes = get_post_meta( $post->ID, 'commercekit_sg_attr', true );
...
function commercekit_sg_admin_meta_save( $post_id, $post ) {
	if ( 'ckit_size_guide' === $post->post_type ) {
		...
		if ( isset( $_POST['commercekit_sg_prod_tmp'] ) && 1 === (int) $_POST['commercekit_sg_prod_tmp'] ) { // phpcs:ignore
		...
		if ( isset( $_POST['commercekit_sg_tag_tmp'] ) && 1 === (int) $_POST['commercekit_sg_tag_tmp'] ) { // phpcs:ignore
			$tags = isset( $_POST['commercekit_sg_tag'] ) ? map_deep( wp_unslash( $_POST['commercekit_sg_tag'] ), 'sanitize_text_field' ) : array(); // phpcs:ignore
			$tags = implode( ',', array_filter( $tags ) );
			update_post_meta( $post->ID, 'commercekit_sg_tag', $tags );
		}
		if ( isset( $_POST['commercekit_sg_attr_tmp'] ) && 1 === (int) $_POST['commercekit_sg_attr_tmp'] ) { // phpcs:ignore
			$attributes = isset( $_POST['commercekit_sg_attr'] ) ? map_deep( wp_unslash( $_POST['commercekit_sg_attr'] ), 'sanitize_text_field' ) : array(); // phpcs:ignore
			$attributes = implode( ',', array_filter( $attributes ) );
			update_post_meta( $post->ID, 'commercekit_sg_attr', $attributes );
		}
	}
}
...
function commercekit_sg_prepare_active_post() {
	...
	$sg_where   = array();
	$sg_where[] = '(pm1.meta_key = \'commercekit_sg_prod\' AND FIND_IN_SET(' . intval( $product_id ) . ', pm1.meta_value))';
	$left_join  = ' LEFT JOIN ' . $wpdb->prefix . 'postmeta AS pm1 ON pm1.post_id = p.ID ';
	if ( count( $categories ) ) {
		$or_qry = array();
		foreach ( $categories as $category_id ) {
			$or_qry[] = 'FIND_IN_SET(' . intval( $category_id ) . ', pm2.meta_value)';
		}
		$sg_where[] = '(pm2.meta_key = \'commercekit_sg_cat\' AND ( ' . implode( ' OR ', $or_qry ) . ' ))';
		$left_join .= ' LEFT JOIN ' . $wpdb->prefix . 'postmeta AS pm2 ON pm2.post_id = p.ID ';
	}
	if ( count( $attr_terms ) ) {
		$or_qry = array();
		foreach ( $attr_terms as $attr_term ) {
			$or_qry[] = 'FIND_IN_SET(\'' . esc_sql( $attr_term ) . '\', pm3.meta_value)';
		}
		$sg_where[] = '(pm3.meta_key = \'commercekit_sg_attr\' AND ( ' . implode( ' OR ', $or_qry ) . ' ))';
		$left_join .= ' LEFT JOIN ' . $wpdb->prefix . 'postmeta AS pm3 ON pm3.post_id = p.ID ';
	}
	if ( count( $tags ) ) {
		$or_qry = array();
		foreach ( $tags as $tag_id ) {
			$or_qry[] = 'FIND_IN_SET(' . intval( $tag_id ) . ', pm4.meta_value)';
		}
		$sg_where[] = '(pm4.meta_key = \'commercekit_sg_tag\' AND ( ' . implode( ' OR ', $or_qry ) . ' ))';
		$left_join .= ' LEFT JOIN ' . $wpdb->prefix . 'postmeta AS pm4 ON pm4.post_id = p.ID ';
	}

	$sg_sql = 'SELECT p.* FROM ' . $wpdb->prefix . 'posts AS p ' . $left_join . ' WHERE p.post_status = \'publish\' AND p.post_type = \'ckit_size_guide\' AND (' . implode( ' OR ', $sg_where ) . ') ORDER BY p.ID DESC LIMIT 0, 1';
...
function commercekit_sg_wpml_make_duplicate( $master_post_id, $target_lang, $post_array, $target_post_id ) {
	...
	$products  = get_post_meta( $master_post_id, 'commercekit_sg_prod', true );
	$products  = explode( ',', $products );
	$products  = array_filter( $products );
	$nproducts = array();
	if ( is_array( $products ) && count( $products ) ) {
	...
	$categories  = get_post_meta( $master_post_id, 'commercekit_sg_cat', true );
	$categories  = explode( ',', $categories );
	$categories  = array_filter( $categories );
	$ncategories = array();
	if ( is_array( $categories ) && count( $categories ) ) {
	...
	$tags  = get_post_meta( $master_post_id, 'commercekit_sg_tag', true );
	$tags  = explode( ',', $tags );
	$tags  = array_filter( $tags );
	$ntags = array();
	if ( is_array( $tags ) && count( $tags ) ) {
	...
	$attributes  = get_post_meta( $master_post_id, 'commercekit_sg_attr', true );
	$attributes  = explode( ',', $attributes );
	$attributes  = array_filter( $attributes );
	$nattributes = array();
	if ( is_array( $attributes ) && count( $attributes ) ) {
	...
	update_post_meta( $target_post_id, 'commercekit_sg_prod', implode( ',', $nproducts ) );
	update_post_meta( $target_post_id, 'commercekit_sg_cat', implode( ',', $ncategories ) );
	update_post_meta( $target_post_id, 'commercekit_sg_tag', implode( ',', $ntags ) );
	update_post_meta( $target_post_id, 'commercekit_sg_attr', implode( ',', $nattributes ) );
}
