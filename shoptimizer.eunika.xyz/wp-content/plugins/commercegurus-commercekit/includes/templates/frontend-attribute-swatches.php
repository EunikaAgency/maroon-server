...
function commercekit_get_as_options( $options ) {
	...
	$commercekit_as['cgkit_attr_gal']  = isset( $options['pdp_attributes_gallery'] ) && 1 === (int) $options['pdp_attributes_gallery'] && isset( $options['pdp_gallery'] ) && 1 === (int) $options['pdp_gallery'] ? 1 : 0;
...
function commercegurus_as_add_shoptimizer_gallery_image() {
	...
	if ( $product && method_exists( $product, 'is_type' ) && $product->is_type( 'variable' ) ) {
		...
		$attributes_gallery = isset( $options['pdp_attributes_gallery'] ) && 1 === (int) $options['pdp_attributes_gallery'] && isset( $options['pdp_gallery'] ) && 1 === (int) $options['pdp_gallery'] ? true : false;
