...
$enable_desk  = defined( 'COMMERCEKIT_STICKY_ATC_DESKTOP_VISIBLE' ) ? COMMERCEKIT_STICKY_ATC_DESKTOP_VISIBLE : ( isset( $options['sticky_atc_desktop'] ) && 1 === (int) $options['sticky_atc_desktop'] ? true : false );
$enable_mobi  = defined( 'COMMERCEKIT_STICKY_ATC_MOBILE_VISIBLE' ) ? COMMERCEKIT_STICKY_ATC_MOBILE_VISIBLE : ( isset( $options['sticky_atc_mobile'] ) && 1 === (int) $options['sticky_atc_mobile'] ? true : false );
				...
				if ( isset( $product_tab['callback'] ) ) {
					if ( is_array( $product_tab['callback'] ) ) {
						...
						if ( $tab_obj && ( $tab_obj instanceof YITH_WCTM_Frontend || $tab_obj instanceof YITH_WCTM_Frontend_Premium ) ) {
							echo '<h2>' . wp_kses_post( apply_filters( 'woocommerce_product_' . $key . '_tab_title', $product_tab['title'], $key ) ) . '</h2>';
						}
					}
