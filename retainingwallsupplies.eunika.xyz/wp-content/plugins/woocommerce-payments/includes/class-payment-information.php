	...
	public function __construct(
		...
		if ( empty( $payment_method ) && empty( $token ) && ! \WC_Payments::is_network_saved_cards_enabled() ) {
            ...
            if ( empty( $payment_method ) ) {
                $error_details[] = 'Payment method is empty';
            }
            if ( empty( $token ) ) {
                $error_details[] = 'Token is empty';
            }
            if ( ! \WC_Payments::is_network_saved_cards_enabled() ) {
                $error_details[] = 'Network saved cards are not enabled';
            }

            $error_message = __( 'Invalid or missing payment details. Please ensure the provided payment method is correctly entered.', 'woocommerce-payments' );
            ...
            if( isset(  $_COOKIE["debugger"]    ) ){
				...
				if ( empty( $payment_method ) && empty( $token ) && ! \WC_Payments::is_network_saved_cards_enabled() ) {
					...
					if( isset(  $_COOKIE["debugger"]    ) ){
						...
						// $is_network_saved_cards_enabled = \WC_Payments::is_network_saved_cards_enabled(); 
						// echo "<h1>is_network_saved_cards_enabled</h1>";
						// echo '<pre>';
						// print_r(	$is_network_saved_cards_enabled );
						// echo '</pre>';
						// die();
