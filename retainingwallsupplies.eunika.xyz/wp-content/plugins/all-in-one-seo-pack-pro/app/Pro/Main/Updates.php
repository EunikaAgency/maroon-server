...
class Updates extends CommonMain\Updates {
	...
	public function runUpdates() {
		...
		if ( version_compare( $lastActiveProVersion, '4.8.4', '<' ) ) {
			$this->addNotificationForOpenAiUsers();
		}

		if ( version_compare( $lastActiveProVersion, '4.8.6', '<' ) ) {
			$this->addSeoAnalyzerObjectsTable();
			$this->addSeoAnalyzerScanDateColumnForPosts();
			$this->addSeoAnalyzerScanDateColumnForTerms();
		}
	}
	...
	/**
	 * Adds a notification for OpenAI users.
	 *
	 * @since 4.8.4
	 *
	 * @return void
	 */
	private function addNotificationForOpenAiUsers() {
		$options = $this->getRawOptions();
		if ( empty( $options['advanced']['openAiKey'] ) ) {
			return;
		}

		if ( CommonModels\Notification::getNotificationByName( 'open-ai-removed' )->exists() ) {
			return;
		}

		CommonModels\Notification::addNotification( [
			'slug'              => uniqid(),
			'notification_name' => 'open-ai-removed',
			'title'             => __( 'Introducing AI Content', 'aioseo-pro' ),
			'content'           => __( 'We are excited to announce the launch of our new AI Content Generator, allowing you to generate FAQs, Key Points, social posts, SEO titles, meta descriptions, and more!</br></br>The old OpenAI integration has been removed in favour of this new feature, so we can focus on providing you with the best AI experience possible. Your AIOSEO Pro license comes with credits that are included in your subscription, so you can start generating content right away.', 'aioseo-pro' ), // phpcs:ignore Generic.Files.LineLength.MaxExceeded
			'type'              => 'info',
			'level'             => [ 'all' ],
			'button1_label'     => __( 'Learn More', 'aioseo-pro' ),
			'button1_action'    => 'https://aioseo.com/docs/ai-content-generator/',
			'start'             => gmdate( 'Y-m-d H:i:s' )
		] );
	}

	/**
	 * Adds our custom table for the SeoAnalysis/SeoAnalyzer issues/checkers.
	 *
	 * @since 4.8.6
	 *
	 * @return void
	 */
	public function addSeoAnalyzerObjectsTable() {
		$db             = aioseo()->core->db->db;
		$charsetCollate = '';
		...
		// Check for seo analyzer objects table.
		if ( ! aioseo()->core->db->tableExists( 'aioseo_seo_analyzer_objects' ) ) {
			$tableName = $db->prefix . 'aioseo_seo_analyzer_objects';
			...
			aioseo()->core->db->execute(
				"CREATE TABLE {$tableName} (
					...
					`object_subtype` varchar(40) NOT NULL,
					...
					`updated` DATETIME NOT NULL,
					PRIMARY KEY (id),
					KEY ndx_aioseo_seo_analyzer_objects_object (object_id, object_type),
					KEY ndx_aioseo_seo_analyzer_objects_code (code)
				) {$charsetCollate};"
			);
		}
	}
	...
	/**
	 * Adds the seo analyzer scan date column to our posts table.
	 *
	 * @since 4.8.6
	 *
	 * @return void
	 */
	public function addSeoAnalyzerScanDateColumnForPosts() {
		if ( ! aioseo()->core->db->columnExists( 'aioseo_posts', 'seo_analyzer_scan_date' ) ) {
			$tableName = aioseo()->core->db->db->prefix . 'aioseo_posts';
			aioseo()->core->db->execute(
				"ALTER TABLE {$tableName}
				ADD seo_analyzer_scan_date datetime DEFAULT NULL AFTER local_seo"
			);
		}
	}

	/**
	 * Adds the seo analyzer scan date column to our terms table.
	 *
	 * @since 4.8.6
	 *
	 * @return void
	 */
	public function addSeoAnalyzerScanDateColumnForTerms() {
		if ( ! aioseo()->core->db->columnExists( 'aioseo_terms', 'seo_analyzer_scan_date' ) ) {
			$tableName = aioseo()->core->db->db->prefix . 'aioseo_terms';
			aioseo()->core->db->execute(
				"ALTER TABLE {$tableName}
				ADD seo_analyzer_scan_date datetime DEFAULT NULL AFTER local_seo"
