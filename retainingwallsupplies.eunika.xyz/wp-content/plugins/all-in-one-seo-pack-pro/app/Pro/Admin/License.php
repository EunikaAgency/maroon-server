...
class License {
	...
	/**
	 * Activates the plugin by license key.
	 *
	 * @since   4.0.0
	 * @version 4.8.1 Moved bulk of the logic to activateHelper().
	 *
	...
	public function activate() {
		...
		return $this->activateHelper( 'activate', $licenseKey );
	}

	/**
	 * Activate the license by OTP.
	 *
	 * @since 4.8.1
	 *
	 * @param  string $otp The one-time password to activate with.
	 * @return bool        Whether or not it was activated.
	 */
	public function activateWithOtp( $otp ) {
		if ( empty( $otp ) ) {
			return false;
		}

		return $this->activateHelper( 'activate/otp', $otp );
	}

	/**
	 * Activates the plugin by license key or OTP.
	 * This is a helper function for activate() and activateWithOtp().
	 *
	 * @since 4.8.1
	 *
	 ...
	 * @return bool        Whether or not it was activated.
	 */
	private function activateHelper( $route, $key ) {
		$this->internalOptions->internal->license->reset(
			[
				'expires',
				'expired',
		...
		$response = $this->sendLicenseRequest( $route, $key, [ $domains ] );
		...
		if ( ! empty( $response->key ) ) {
			$this->options->general->licenseKey = $response->key;
		}
	...
	public function getLicenseKey() {
		// Subsite activated licenses take precedence over network activated licenses.
		if ( aioseo()->options->general->licenseKey ) {
			return aioseo()->options->general->licenseKey;
		}

		// If no subsite license is found, check for a network license.
